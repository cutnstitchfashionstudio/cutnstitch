...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ── Resolve user ID from server-side JWT cookie (100% reliable) ──────────
    // The client-sent userId is intentionally ignored — it can be stale, missing,
    // or a literal '${user.id}' string due to template-literal escaping on the client.
    // We read the authenticated user directly from their httpOnly auth_token cookie.
    let userId = null;
    ...
    if (token) {
      ...
      } catch (_) { /* Invalid token — userId stays null */ }
    }

    // Last-resort fallback: accept client-sent userId only if it looks valid
    if (!userId) {
      const clientId = (req.body.userId || '').toString().trim();
      if (clientId && clientId !== 'undefined' && clientId !== 'null' && !clientId.includes('${')) {
        userId = clientId;
      }
    }
    ...
    // 2. Check email is not already in use by a DIFFERENT user
    ...
    if (pendingResult) {
      // Move from Pending → Users with email added (instant verification)
      ...
      // Already in Users — just update the email field
    ...
    // 4. Issue a fresh JWT with the linked email
    ...
    return res.status(200).json({
      ...
      user: { id: finalUser.ID, name: finalUser.Name, email: finalUser.Email, phone: finalUser.Phone, isVerified: true }
    });
