const { findRow, appendRow, updateRow, generateId } = require('../../sheets');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const profileRes = await fetch(
      `https://graph.facebook.com/me?fields=id,name,email,gender&access_token=${tokenData.access_token}`
    );
    const profile = await profileRes.json();

    const emailToUse = profile.email || `${profile.id}@facebook.com`;

    // 3. Find or create user in Google Sheets (Users tab — Facebook OAuth = verified immediately)
    let userRow = null;
    const existing = await findRow('Users', 'Email', emailToUse);

    if (existing) {
      userRow = existing.data;
      // No update needed for Google Sheets (no Gender column in schema)
    } else {
      userRow = {
        ID:           generateId(),
    ...
    res.end(`
      ...
      <body style="background:#0a0f1e;color:#e2e8f0;font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;">
        ...
        <script>
          if (window.opener) {
            try { window.opener.postMessage({ type: 'oauth-success' }, window.location.origin); window.close(); }
            catch (e) { window.location.href = '/portal'; }
