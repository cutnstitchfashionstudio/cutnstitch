...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const phoneVerified = isVerified
      ? (result.data.PhoneVerified || '').toString().toUpperCase() === 'TRUE'
      : false;

    res.status(200).json({
      authenticated: true,
      user: { id: decoded.id, name: decoded.name, email: dbEmail, phone, isVerified, phoneVerified }
    });
  } catch (err) {
